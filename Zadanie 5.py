...
prof = res - costs
# Пользователь задёт переменные выручки и издержки, в третьей строчке мы задаём переменную прыбыли
if prof > 0:
    ...
    # Первое строчка это условие при котором прибыль больше нуля, мы выводим значение прибыли на экран,
    # задаём переменную рентабельности и третье выводим переменное рентабельность на экран
    emp_num = int(input("Введите численность сотрудников фирмы: "))
    emp_pr = prof / emp_num
    print(f"Прибыль фирмы в расчете на одного сотрудника = {emp_pr}")
# Пользователь задаёт переменную колличество сотрудников фирмы,
# во творой строчке мы задаём переменную прибыли на каждого сотрудника и третья строчка вы водим на экран
elif prof < 0:
    print(f"Финансовый результат - убыток. Ее величина: {prof}")
# задаём условие при котором прибыль меньше нуля
else:
